...
async def get_my_score(ctx):
    user_id = ctx.author.id
    collage_path = manager.create_collage(user_id)

    if collage_path:
        with open(collage_path, 'rb') as img:
            file = discord.File(img, filename="collage.jpg")
            await ctx.send("İşte kazandığınız ve gizlenmiş ödülleriniz!", file=file)
    else:
        await ctx.send("Henüz kazanılmış bir ödülünüz yok.")
